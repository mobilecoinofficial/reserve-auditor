...
type TProps = {
    auditedUnwrap: TAuditedBurn
}

export const AuditedUnwrap: FC<TProps> = ({ auditedUnwrap }: TProps) => {
    return (
        <Grid container >
            <Grid item xs={2} data-row="type">RSV Withdrawn</Grid>
            {/* TODO: fetch token ticker from live data*/}
            <Grid item xs={2} data-row="amount">{auditedUnwrap.withdrawal.amount} RSV</Grid>
            {/* TODO: fetch org name from live data */}
            <Grid item xs={2} data-row="lp">ExampleOrg</Grid>
            <Grid item xs={5} data-row="hash">{auditedUnwrap.withdrawal.ethTxHash}</Grid>
            ...
            {/* TODO: fetch token ticker from live data*/}
            <Grid item xs={2} data-row="amount">{auditedUnwrap.burn.amount} eUSD</Grid>
            <Grid item xs={2} data-row="lp">--</Grid>
            <Grid item xs={5} data-row="hash">{auditedUnwrap.burn.publicKeyHex}</Grid>
