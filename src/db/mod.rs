...
/// Reserve Auditor Database.
#[derive(Clone)]
pub struct ReserveAuditorDb {
    pool: Pool<ConnectionManager<SqliteConnection>>,
    logger: Logger,
}

impl ReserveAuditorDb {
    ...
    /// Sync reserve audit data of a single block.
    pub fn sync_block(
        &self,
        block: &Block,
...
mod tests {
    ...
    fn test_sync_block_happy_flow(logger: Logger) {
        ...
        let (sync_block_data, block_index) = append_and_sync(
            block_contents,
            &mut ledger_db,
            &reserve_auditor_db,
            &mut rng,
        ...
        let (mut sync_block_data, block_index) = append_and_sync(
            block_contents,
            &mut ledger_db,
            &reserve_auditor_db,
            &mut rng,
    ...
    fn test_sync_block_counts_mint_txs_without_active_config(logger: Logger) {
        ...
        // the reserve auditor db number of blocks synced gets out of sync and we will
        ...
        let (_, block_index) = append_and_sync(
            block_contents,
            &mut ledger_db,
            &reserve_auditor_db,
            &mut rng,
