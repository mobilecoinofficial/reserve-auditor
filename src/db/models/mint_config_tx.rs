...
/// This stores transaction details for a mint configuration.
#[derive(
...
impl MintConfigTx {
    ...
    pub fn from_core_mint_config_tx(
    ...
    pub fn insert_from_core_mint_config_tx(
        ...
        let mut mint_config_tx =
            Self::from_core_mint_config_tx(block_index, block_timestamp, config_tx);
...
mod tests {
    ...
    fn most_recent_for_token_works(logger: Logger) {
        ...
        MintConfigTx::insert_from_core_mint_config_tx(7, None, &mint_config_tx2, &conn).unwrap();
    ...
    fn insert_enforces_uniqueness(logger: Logger) {
        ...
        assert!(
            MintConfigTx::insert_from_core_mint_config_tx(5, None, &mint_config_tx2, &conn)
                .is_err()
        );

        // Trying for a different block but with the same nonce will fail.
        assert!(
            MintConfigTx::insert_from_core_mint_config_tx(6, None, &mint_config_tx1, &conn)
                .is_err()
        );

        // Sanity, inserting a different mint config at block index 6 should succeed.
        assert!(
            MintConfigTx::insert_from_core_mint_config_tx(6, None, &mint_config_tx2, &conn).is_ok()
        );
    }
    ...
    fn get_total_minted_before_block_works(logger: Logger) {
        ...
        // Create a few test mint config txs and insert them.
        ...
        MintTx::insert_from_core_mint_tx(3, None, mint_config_id1, &mint_tx1, &conn).unwrap();

        let mint_tx2 = create_mint_tx(token_id1, &signers1, 200, &mut rng);
        MintTx::insert_from_core_mint_tx(6, None, mint_config_id1, &mint_tx2, &conn).unwrap();

        let mint_tx3 = create_mint_tx(token_id1, &signers1, 300, &mut rng);
        MintTx::insert_from_core_mint_tx(8, None, mint_config_id1, &mint_tx3, &conn).unwrap();

        let mint_tx4 = create_mint_tx(token_id1, &signers2, 400, &mut rng);
        MintTx::insert_from_core_mint_tx(11, None, mint_config_id2, &mint_tx4, &conn).unwrap();

        let mint_tx5 = create_mint_tx(token_id2, &signers3, 2000, &mut rng);
        MintTx::insert_from_core_mint_tx(11, None, mint_config_id3, &mint_tx5, &conn).unwrap();
        ...
        MintTx::insert_from_core_mint_tx(12, None, mint_config_id3, &mint_tx6, &conn).unwrap();
