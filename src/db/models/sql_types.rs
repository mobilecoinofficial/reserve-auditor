...
use crate::gnosis::{EthAddr, EthTxHash, EthTxValue};
use diesel::{
    backend::Backend,
...
#[serde(transparent)]
pub struct SqlEthTxValue(EthTxValue);

impl Deref for SqlEthTxValue {
    type Target = EthTxValue;
...
impl From<EthTxValue> for SqlEthTxValue {
    fn from(src: EthTxValue) -> Self {
        Self(src)
    }
}
...
impl fmt::Display for SqlEthTxValue {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.0.to_string())
    }
}

impl<DB: Backend> ToSql<diesel::sql_types::Text, DB> for SqlEthTxValue {
    fn to_sql<W: Write>(&self, out: &mut Output<W, DB>) -> serialize::Result {
        <String as ToSql<diesel::sql_types::Text, DB>>::to_sql(&self.0.to_string(), out)
    }
}

impl<DB> FromSql<diesel::sql_types::Text, DB> for SqlEthTxValue
where
    DB: Backend,
    String: FromSql<diesel::sql_types::Text, DB>,
{
    fn from_sql(src: Option<&DB::RawValue>) -> deserialize::Result<Self> {
        ...
        Ok(SqlEthTxValue(
            EthTxValue::from_str(&str).map_err(|e| e.to_string())?,
        ))
    }
}
